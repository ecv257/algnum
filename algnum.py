...
por el momento usamos el gcd de fractions.
...
def isqrt(x):
	...
	n = 1 << (nsigbits(x)+1)>>1
	m = (n+x//n)>>1
	while m < n: n,m= m,(n+x//n)>>1
	return n
	
def issquare(x):
	if type(x)==int:
		t=isqrt(x) if x>0 else 0
		return t*t==x
	elif type(x)==Fraction:
		return issquare(x.numerator) and issquare(x.denominator)
...
class factor:
	...
	def divisors(self):
		...
		if len(self.P):
			...
			yield 1
	def __str__(self):
		return '*'.join([str(c[0])+('' if c[1]==1 else '^'+str(c[1])) for c in self.P])
...
class poly(tuple):
	...
	deg = property(operator.methodcaller('__len__'))
